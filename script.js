...
function confirmCrop() {
    ...
    const canvas = activeCropper.getCroppedCanvas({ width: 300, height: 400, imageSmoothingQuality: 'high' });
...
function setupFirebaseListener() {
    database.ref('records').on('value', (snapshot) => {
        ...
        if(snapshot.exists()) {
            ...
            newDb = Object.keys(data).map(key => {
                let item = data[key];
                item.firebaseKey = key;
                if (item.history && typeof item.history === 'object' && !Array.isArray(item.history)) {
        ...
        database.ref('config/staffList').once('value', (confSnap) => {
            let sList = confSnap.val() || [];
            newDb.push({ id: 'config_staff', type: 'config', staffList: sList });
...
function hardRefresh() { 
    ...
    database.ref('records').once('value').then(() => {
        document.getElementById('sync-status').innerText = "Cloud Synced";
...
async function addCustomer() {
    ...
    let newKey = database.ref('records').push().key;
    cust.firebaseKey = newKey;
    db.push(cust); 
    saveAndRender(); 
    database.ref('records/' + newKey).set(cust);
...
function savePayment() { 
    ...
    recalculateCase(c); 
    saveAndRender(); 
    if(c.firebaseKey) database.ref('records/' + c.firebaseKey + '/history').set(c.history); 
...
function saveBulkPayment() { 
    ...
    if(hasDuplicate) { triggerShake('bulk-start-date'); triggerShake('bulk-end-date'); return showToast(i18n[currentLang].dupEntry || "Payment already added for this date!"); } 
...
async function saveEdit() { 
    ...
    saveAndRender(); 
    if(c.firebaseKey) database.ref('records/' + c.firebaseKey).update(c); 
    showToast("Account Updated"); 
}

function toggleArchiveUI(id) { let c = db.find(x => x.id === id); let isArchiving = !c.isArchived; let msg = isArchiving ? "Move to Archive (Closed Cases)?" : "Restore to Active Cases?"; askConfirm(msg, () => { c.isArchived = isArchiving; saveAndRender(); if(c.firebaseKey) database.ref('records/' + c.firebaseKey).update({isArchived: isArchiving}); showToast(isArchiving ? (i18n[currentLang].archiveToast || "Archived!") : (i18n[currentLang].unarchiveToast || "Unarchived!")); }); }
function deleteCustUI(id) { askConfirm("Are you sure you want to delete this entire account?", () => { let c = db.find(x => x.id === id); db = db.filter(x => x.id !== id); saveAndRender(); if(c && c.firebaseKey) database.ref('records/' + c.firebaseKey).remove(); showToast("Account Deleted!"); }); }
function deleteHistoryUI(custId, originalIndex) { askConfirm("Delete this entry?", () => { let c = db.find(x => x.id === custId); c.history.splice(originalIndex, 1); recalculateCase(c); saveAndRender(); if(c.firebaseKey) database.ref('records/' + c.firebaseKey + '/history').set(c.history); showToast("Entry Deleted!"); }); }
function deleteSelectedHistoryUI(id) { let checks = document.querySelectorAll(`.del-chk-${id}:checked`); if(checks.length === 0) return toggleMultiDel(id); askConfirm(`Delete ${checks.length} entries?`, () => { let c = db.find(x => x.id === id); let indices = Array.from(checks).map(ck => parseInt(ck.value)).sort((a,b) => b-a); indices.forEach(idx => { c.history.splice(idx, 1); }); multiDelMode[id] = false; recalculateCase(c); saveAndRender(); if(c.firebaseKey) database.ref('records/' + c.firebaseKey + '/history').set(c.history); showToast("Selected Deleted!"); }); }
                ...
                <div class="card-header">
                    <div style="min-width:0; flex:1; display:flex; align-items:center; gap:12px;">
                        ...
                        <div style="min-width:0;">
                            <div class="c-name">${c.name} ${c.staffRef?`<span style="color:var(--accent-orange); font-size:10px; margin-left:5px; flex-shrink:0;">[Ref: ${c.staffRef}]</span>`:''} ${(isOwnerMode && c.isPersonal)?'<span style="flex-shrink:0;">👑</span>':''}</div>
